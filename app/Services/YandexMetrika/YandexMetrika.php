...
use App\Services\YandexMetrika\Requests\CleanRequest;
use App\Services\YandexMetrika\Contracts\ParamRequest;
use App\Services\YandexMetrika\Requests\CancelRequest;
use App\Services\YandexMetrika\Requests\CreateRequest;
use App\Services\YandexMetrika\Requests\LogListRequest;
use App\Services\YandexMetrika\Requests\MetrikaRequest;
use App\Services\YandexMetrika\Responses\CleanResponse;
use App\Services\YandexMetrika\Requests\DownloadRequest;
use App\Services\YandexMetrika\Responses\CancelResponse;
use App\Services\YandexMetrika\Responses\CreateResponse;
use App\Services\YandexMetrika\Responses\LogListResponse;
use App\Services\YandexMetrika\Responses\MetrikaResponse;
use App\Services\YandexMetrika\Requests\CapabilityRequest;
use App\Services\YandexMetrika\Responses\DownloadResponse;
use App\Services\YandexMetrika\Requests\InformationRequest;
use App\Services\YandexMetrika\Responses\CapabilityResponse;
use App\Services\YandexMetrika\Responses\InformationResponse;
 ...
 *
 * @package App\Services\YandexMetrika
 */
class YandexMetrika
{
    ...
    public function __construct()
    {
        ...
        $counterId = (int)config('yandex-metrika-api.counter_id');
     ...
     */
    public function setCounter(string $token, $counterId, int $cacheLifetime = null): YandexMetrika
    {
     ...
     *
     * @param App\Services\YandexMetrika\Contracts\Request $request
    ...
    /**
     * Количество посетителей профиля Я.Карты с разбивкой по адресам, сервисам и событиям
     *
     * @param DateTime $startDate
     * @param DateTime $endDate
     * @return MetrikaResponse
     * @throws \GuzzleHttp\Exception\GuzzleException|Exception
     */
    public function getVisitsUsersYandexProfileForPeriodAddressService(DateTime $startDate, DateTime $endDate ): MetrikaResponse
    {
        $request = (new MetrikaRequest($this->counterId))
            ->setDate1($startDate)
            ->setDate2($endDate)
            ->setMetrics(['ym:s:vacuumevents'])
            ->setDimensions(['ym:s:vacuumOrganization','ym:s:vacuumSurface','ym:s:vacuumEvent']);

        $cacheName = md5(serialize('visits-users-yandexProfile-address-service' . $startDate->format('Y-m-d') . $endDate->format('Y-m-d')));
    ...
    /**
     * Количество посетителей профиля Я.Карты с разбивкой по адресам и типом устройства
     *
     * @param DateTime $startDate
     * @param DateTime $endDate
     * @return MetrikaResponse
     * @throws \GuzzleHttp\Exception\GuzzleException|Exception
     */
    public function getVisitsUsersYandexProfileForPeriodDevice(DateTime $startDate, DateTime $endDate ): MetrikaResponse
    {
        $request = (new MetrikaRequest($this->counterId))
            ->setDate1($startDate)
            ->setDate2($endDate)
            ->setMetrics(['ym:s:vacuumevents'])
            ->setDimensions(['ym:s:vacuumOrganization','ym:s:deviceCategory']);

        $cacheName = md5(serialize('visits-users-yandexProfile-address-device' . $startDate->format('Y-m-d') . $endDate->format('Y-m-d')));
    ...
    /**
     * Количество посетителей профиля Я.Карты с разбивкой по адресам и производителем моб устройства
     *
     * @param DateTime $startDate
     * @param DateTime $endDate
     * @return MetrikaResponse
     * @throws \GuzzleHttp\Exception\GuzzleException|Exception
     */
    public function getVisitsUsersYandexProfileForPeriodDeviceManufacture(DateTime $startDate, DateTime $endDate ): MetrikaResponse
    {
        $request = (new MetrikaRequest($this->counterId))
            ->setDate1($startDate)
            ->setDate2($endDate)
            ->setMetrics(['ym:s:vacuumevents'])
            ->setDimensions(['ym:s:vacuumOrganization','ym:s:mobilePhone']);

        $cacheName = md5(serialize('visits-users-yandexProfile-address-device-manufacture' . $startDate->format('Y-m-d') . $endDate->format('Y-m-d')));
    ...
    /**
     * Количество посетителей визитов с разбивкой по источникам и группировкой достижения целей
     *
     * @param DateTime $startDate
     * @param DateTime $endDate
     * @return MetrikaResponse
     * @throws \GuzzleHttp\Exception\GuzzleException|Exception
     */
    public function getVisitsUsersSourceGoalForPeriod(DateTime $startDate, DateTime $endDate, Array $metrics ): MetrikaResponse
    {
        $request = (new MetrikaRequest($this->counterId))
            ->setDate1($startDate)
            ->setDate2($endDate)
            ->setMetrics($metrics)
            ->setDimensions(['ym:s:lastSignTrafficSource']);
            

        $cacheName = md5(serialize('visits-users-source-goal' . $startDate->format('Y-m-d') . $endDate->format('Y-m-d')));
    ...
    /**
     * Количество достижения целей с разбивкой по устройствам
     *
     * @param DateTime $startDate
     * @param DateTime $endDate
     * @return MetrikaResponse
     * @throws \GuzzleHttp\Exception\GuzzleException|Exception
     */
    public function getReachGoalDevicesForPeriod(DateTime $startDate, DateTime $endDate, Array $metrics ): MetrikaResponse
    {
        $request = (new MetrikaRequest($this->counterId))
            ->setDate1($startDate)
            ->setDate2($endDate)
            ->setMetrics($metrics)
            ->setDimensions(['ym:s:deviceCategory']);
